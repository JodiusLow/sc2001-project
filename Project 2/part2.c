...
void heapify(struct HeapNode* arr[], int n, int i) //minHeap
{
    int smallest = i;
    int left = 2 * i + 1;
    int right = 2 * i + 2;
...
void decreaseKey(struct HeapNode* arr[], int heapSize, int source, int dist) 
//update the distance value of source node and ensure min heap property is maintained
{
    int i;
    ...
    if (i < heapSize) 
    {
        arr[i]->distance = dist;//update the distance value of source node
...
void dijkstra(struct Graph* graph, int source) 
{
    ...
    dist[source] = 0;
    decreaseKey(heap, heapSize, source, dist[source]);
    ...
    while (heapSize > 0) 
    {
        ...
        while (current != NULL) 
        {
            ...
            if (dist[u] != INT_MAX && (dist[u] + weight < dist[v])) //finding shortest distance from source to vertex
            {
                ...
                decreaseKey(heap, heapSize, v, dist[v]); //sort priority queue using fixHeap
            }
...
int main() 
{
    ...
    for (int V = 1000; V<=maxVertices; V+=1000)
    {
        struct Graph* graph = createGraph(V);
        int E = 0;

        // //Generate complete (fully connected) graph
        // for (int i = 0; i < V; i++) 
        // {
        //     for (int j = 0; j< V/100; j++) // only use a fraction of the total edges 
        //     {   
        //         if (i!=j) // no self loop
        //         {
        ...
        //     }
        // }
         // Generate sparse graph with each node having only one child
        for (int i = 0; i < V - 1; i++) // edge will be V - 1
        {
            // Randomly select a single child for each node
            int child = i + 1 + rand() % (V - i);
            int weight = 1 + rand() % (INT_MAX);
            addEdge(graph, i, child, weight);
            E++;
        }
        ...
        printf("Dense Graph with %d vertices and %d edges: Time taken: %f seconds\n", V, E, cpu_time_used);
